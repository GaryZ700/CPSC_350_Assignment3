...
/*Overloaded GetFile Function 
  returns an fstream with an open file that the user provided
  fileName: string of file name to validate to ensure it is a valid file name
  msg: string of a message to display to the user to get a file name
  error_msg: string of a message to display to the user if the file name is invalid  
  mode: openmode that represents the mode that the file should be opened in 
*/
fstream UserInput::GetFile(string fileName, string msg, string errorMsg, ios::openmode mode){

	fstream file;
	file.open(fileName);
	
	//if the provided file name is valid, then return the file
	//otherwise prompt the user to enter a new file name
	if(file.is_open())
		return file;
	else
		return GetFile(msg, errorMsg, mode);
}
